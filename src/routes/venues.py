...
VENUE_COLUMNS = (
        "id, uid, created_at, updated_at, org_uid, name, "
...
async def create_venue(venue:VenueCreateRequest, db: DBSession, user:CurrentUser):
    ...
    row: Record = await db.fetchrow(f"""
                    insert into venues(uid, org_uid, creator_user_uid, name, location)
                        values ($1, $2, $3, $4, $5)
                    returning {VENUE_COLUMNS}
            """, uuid4(), venue.org_uid, user.uid, venue.name, venue.location.to_wkt())

    return VenueResponse(**row)

@router.get("/venues", response_model=VenueListResponse)
async def list_venues(db: DBSession, after: int = 0, limit: Annotated[int, Query(ge=1, le=100)] = 10):
    rows: list[Record] = await db.fetch(f"""select {VENUE_COLUMNS} from venues where id > $1 order by id asc limit $2""", after, limit)
...
async def get_venues(uid:UUID, db: DBSession):
    row: Record = await db.fetchrow(f"""select {VENUE_COLUMNS} from venues where uid=$1 limit 1""", uid)
    if not row:
